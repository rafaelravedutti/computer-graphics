...
static void loadMesh(Mesh& mesh, const std::string& file)
{
    //load teapot
    ...
    createBuffers(mesh,objLoader.outVertices,indices,GL_TRIANGLES);
}
...
CG::CG(int w, int h) : Window(w,h)
{
    ...
    loadMesh(teapotMesh,"data/teapot.obj");
    loadMesh(lanternMesh,"data/lantern.obj");
    loadMesh(lanternLampMesh,"data/lanternlamp.obj");
    loadMesh(bonfireMesh,"data/bonfire.obj");
    loadMesh(armadilloMesh,"data/armadillo.obj");

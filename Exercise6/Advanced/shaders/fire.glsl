...
void main()
{
    ...
    float alphaNoise = snoise(vec3(x*2,y*2 + time,time*0.3)) * 0.5 + 0.5;
    //    float alphaShape  =  (1.0f - (x * x));

    float alphaShape  =  smoothstep(-1.0,-0.0,x) * smoothstep(1.0,0.0,x);
    alphaShape *= smoothstep(0.0,1.0,y);
    //    alphaShape *= y;
    float alpha = alphaShape;
    ...
    vec2 offset = vec2(OctavePerlin(tc2.x,tc2.y,time,3,0.2),
                       OctavePerlin(tc2.x,tc2.y,time,3,0.2));

    offset *= 0.2;
    offset.y += time * 3;

    fc += offset * 1;
    //    vec2 fc = vec2(tc.x,tc.y)*7;
    float fireNoise = OctavePerlin(fc.x,fc.y,time*2,4,0.4);
    fireNoise = clamp(fireNoise,-1,1) * 1.5 + 1.5;


    //    fireNoise = fireNoise * fireNoise;
    ...
    //    out_color = vec4(fireNoise);
    out_color = vec4(fireColor,finalAlpha );
    //        out_color = vec4(vec3(alphaShape),1);
}
